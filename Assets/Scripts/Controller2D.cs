...
public class Controller2D : RaycastController
{
    ...
    void ClimbSlope(ref Vector2 moveAmount, float slopeAngle, Vector2 slopeNormal)
    {
        ...
        if (moveAmount.y <= climbVelocityY)
        {
            ...
            moveAmount.x = Mathf.Cos(slopeAngle * Mathf.Deg2Rad) * Mathf.Sign(moveAmount.x) * moveDistance;
    ...
    void SlideDownMaxSlope(RaycastHit2D hit, ref Vector2 moveAmount)
    {
        if (hit)
        {
            ...
            {
                float directionX = Mathf.Sign(hit.normal.x);
                moveAmount.x = directionX * (Mathf.Abs(moveAmount.y) - hit.distance) / Mathf.Tan(slopeAngle * Mathf.Deg2Rad);
                collisions.slopeAngle = slopeAngle;
